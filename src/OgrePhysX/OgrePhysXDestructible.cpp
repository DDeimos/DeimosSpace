...
namespace OgrePhysX
{
		...
		: mPointRenderable(nullptr), mBreakForce(breakForce), mBreakTorque(breakTorque), mDensity(density), mScale(scale)
	{
		...
		float breakForceScale = scale.x*scale.y*scale.z*density;
		...
		while(childElement)
		{
			SplitPart *part1 = getOrCreatePart(childElement->Attribute("name"), childElement->Attribute("material"));
			TiXmlElement* connectionElement = childElement->FirstChildElement();
			while(connectionElement)
			{
				SplitPart *part2 = getOrCreatePart(connectionElement->Attribute("target"), connectionElement->Attribute("material"));

				TiXmlElement* attributeElement = connectionElement->FirstChildElement();
				double breakForce = mBreakForce;
				double breakTorque = mBreakTorque;
				while(attributeElement)
				{
					double d;
					if (attributeElement->GetText()/*ValueStr()*/ == "rel_force")
					{
						 attributeElement->Attribute("value", &d);
						 breakForce *= d;
					}
					if (attributeElement->GetText()/*ValueStr()*/ == "rel_torque")
					{
						 attributeElement->Attribute("value", &d);
						 breakTorque *= d;
					}
				...
				//connect the parts
				createJoint(part1, part2, (float)breakForce, (float)breakTorque);
	...
	Destructible::SplitPart::SplitPart(Destructible *destructible, const Ogre::String &meshName, const Ogre::String &materialName)
	{
	...
	Destructible::SplitPart* Destructible::getOrCreatePart(const Ogre::String &meshName, const Ogre::String &materialName)
	{
		auto find = mParts.find(meshName);
		if (find != mParts.end()) return find->second;

		SplitPart *part = new SplitPart(this, meshName, materialName);
